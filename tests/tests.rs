use std::{ fs, io::Read };
use workspace_opener::workspace_opener_lib::model::{
    AppConfig,
    Item,
    Preset,
    PresetInfo,
...
fn preset_creation_large() {
    ...
    let target = Preset::new(String::from("Test Preset"), 4, vec![2, 1, 1, 20], args, preset_info);
...
fn test_write_to_file_and_create() {
    ...
    let test_string =
        "{\"presets\":[{\"name\":\"Test Preset\",\"tabs\":3,\"windows\":[2,1,1],\"args\":[\"arg w1\",\"arg w1\",\"arg w2\",\"arg w3\"],\"preset_info\":{\"wt_profile\":\"\",\"init_shell\":\"powershell\",\"target_shell\":\"powershell\"}}],\"settings\":{\"debug_mode\":false}}".to_string();

    app_config
        .write_preset_to_file(&app_messages, WriteType::Create, config_path)
...
fn test_write_to_file_and_delete() {
    ...
    app_config.delete_preset_by_name("Test Preset").expect("Failed to delete preset");

    let test_string = "{\"presets\":[],\"settings\":{\"debug_mode\":false}}".to_string();

    app_config
        .write_preset_to_file(&app_messages, WriteType::Edit, config_path)
        .expect("Failed to write file");
...
fn test_into_items() {
    ...
    let wt_profile = Item::new(
        ...
        State::ChangeFieldName,
        Some(PresetValue::PresetInfo(PresetInfoValue::WtProfile("".to_string())))
    );

    target.push(wt_profile);

    let init_shell = Item::new(
        format!("Init shell (powershell/pwsh/cmd): {}", "powershell".to_string()),
        State::ChangeFieldName,
        Some(PresetValue::PresetInfo(PresetInfoValue::InitShell(ShellType::WindowsPowershell)))
    );

    target.push(init_shell);

    let target_shell = Item::new(
        format!("Target shell (powershell/pwsh/cmd/bash/zsh/fish): {}", "powershell".to_string()),
        State::ChangeFieldName,
        Some(PresetValue::PresetInfo(PresetInfoValue::TargetShell(ShellType::WindowsPowershell)))
    );
...
fn test_preset_change_field_value() {
    ...
    preset.change_field_value(PresetValue::Tabs(5)).expect("Failed changing preset's tabs");
    ...
    preset.change_field_value(PresetValue::Tabs(2)).expect("Failed changing preset's tabs");

    let target = Preset::new(
        String::from("Preset Changed"),
        2,
        vec![2, 1],
        vec![String::from("arg w1"), String::from("arg w1"), String::from("arg w2")],
        preset.get_preset_info()
    );
    ...
    preset
        .change_field_value(
            PresetValue::PresetInfo(PresetInfoValue::WtProfile("Ubuntu".to_owned()))
        )
        .expect("Failed changing preset's windows");

    preset
        .change_field_value(PresetValue::PresetInfo(PresetInfoValue::TargetShell(ShellType::Zsh)))
        .expect("Failed changing preset's windows");
...
fn wt_command_default() {
    ...
    let target =
        "wt.exe powershell -NoExit -Command 'arg w1\\;'`; sp powershell -NoExit -Command 'arg w1\\;'`; nt powershell -NoExit -Command 'arg w2\\;'`; nt powershell -NoExit -Command 'arg w3\\;'";

    assert_eq!(app_config.create_wt_command("Test Preset").unwrap().1, target);
}
...
fn wt_command_ubuntu_bash() {
    ...
    let target =
        "wt.exe -p \"Ubuntu\" wsl ~ -e bash -l -i -c 'ls\\;exec bash\\;'`; nt -p \"Ubuntu\" wsl ~ -e bash -l -i -c 'ls\\;exec bash\\;'`; sp -p \"Ubuntu\" wsl ~ -e bash -l -i -c 'ls\\;exec bash\\;'";
...
fn wt_command_3tabs_1windows() {
    ...
    let target =
        "wt.exe powershell -NoExit -Command 'ls\\;'`; nt powershell -NoExit -Command 'pwd\\;'`; nt powershell -NoExit -Command 'cd ..\\;'";
...
fn wt_command_2tabs_2windows() {
    ...
    let preset = Preset::new(
        ...
        2,
        vec![2, 2],
        vec!["ls".to_string(), "pwd".to_string(), "cd ..".to_string(), "ls".to_string()],
        preset_info
    );
    ...
    let target =
        "wt.exe powershell -NoExit -Command 'ls\\;'`; sp powershell -NoExit -Command 'pwd\\;'`; nt powershell -NoExit -Command 'cd ..\\;'`; sp powershell -NoExit -Command 'ls\\;'";
...
fn wt_command_2tabs_3windows_1window() {
    ...
    let target =
        "wt.exe powershell -NoExit -Command 'ls\\;'`; sp -s .66 powershell -NoExit -Command 'pwd\\;'`; sp -s .5 powershell -NoExit -Command 'cd ..\\;'`; nt powershell -NoExit -Command 'ls\\;'";
...
fn wt_command_1tabs_4windows() {
    ...
    let target =
        "wt.exe powershell -NoExit -Command 'ls\\;'`; sp powershell -NoExit -Command 'pwd\\;'`; sp powershell -NoExit -Command 'cd ..\\;'`; mf left`; sp powershell -NoExit -Command 'ls\\;'";
